...
namespace ETong.ETM.ClientLog.Api.Common
{
    ...
    public sealed class MongoDBHelper
    {
        ...
        /// <returns></returns>
        public static long GetTotal(string collectionName, IMongoQuery query)
        {
            return GetTotal(MongoDBHelper.connectionString_Default, MongoDBHelper.database_Default, collectionName, query);
        }
        ...
        /// <param name="query"></param>
        /// <returns></returns>
        public static long GetTotal(string connectionString, string databaseName, string collectionName, IMongoQuery query)
        {
            long Total = 0;
            MongoServer server = new MongoClient(connectionString).GetServer();
            //获取数据库或者创建数据库（不存在的话）。
            ...
            {
                MongoCollection<BsonDocument> myCollection = database.GetCollection<BsonDocument>(collectionName);
                Total = myCollection.Count(query);
            }
            return Total;
        }
        #endregion
